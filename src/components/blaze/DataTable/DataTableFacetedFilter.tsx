...
import type { Option } from "~/components/blaze/DataTable/types";
            ...
            <div
              ...
              aria-label={
                title !== undefined && title !== ""
                  ? `${typo("Сбросить фильтр")}: ${title}`
                  : typo("Сбросить фильтр")
              }
              tabIndex={0}
              ...
              <div className="hidden items-center gap-1 lg:flex">
                {selectedValues.size > 2 ? (
                  ...
                  options
                    ...
                    .map((option) => (
                      <Badge variant="secondary" key={option.value} className="h-6 min-h-6 rounded-sm px-1 text-xs font-normal">
                        {option.label}
                      </Badge>
                    ))
                )}
      ...
      <PopoverContent className="w-50 p-0" align="start">
        <Command>
          ...
          <CommandList className="max-h-full">
            <CommandEmpty>{typo("Ничего не найдено.")}</CommandEmpty>
              ...
              {options.map((option) => {
                ...
                return (
                  <CommandItem key={option.value} onSelect={() => onItemSelect(option, isSelected)}>
                    <div
                      className={cn(
                        "text-primary-foreground border-primary flex size-4 items-center justify-center rounded-sm border",
