  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button
          aria-label={typo("Видимость колонок")}
          role="combobox"
          variant="outline"
          size="sm"
      ...
      <PopoverContent className="w-50 p-0" {...props}>
        <Command>
          <CommandInput placeholder={typo("Поиск по колонкам…")} />
          <CommandList>
            <CommandEmpty>{typo("Колонки не найдены.")}</CommandEmpty>
            <CommandGroup>
              {columns.map((column) => (
                <CommandItem
                  key={column.id}
